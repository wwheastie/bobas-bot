...
public class CharacterMatchService {
	...
	public CharacterMatches getMatches(final String requestedCharacter, final double proximity) {
		...
		for (final CharacterDto characterDto : characterDtos) {
			double shortNameDistance = jaroWinklerDistance.apply(requestedCharacter, characterDto.getShortName());
			double nameDistance = jaroWinklerDistance.apply(requestedCharacter, characterDto.getName());

			if (exactMatchFound(shortNameDistance, characterDto.getShortName(), requestedCharacter) ||
					exactMatchFound(nameDistance, characterDto.getName(), requestedCharacter)) {
